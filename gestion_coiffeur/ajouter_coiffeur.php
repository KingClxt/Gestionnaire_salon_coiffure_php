...
if(isset($_POST['login'])){
    ...
    $requete_verif = "SELECT * FROM coiffeur WHERE login_coiffeur = ?";
    $prepare_verif = $cnx->prepare($requete_verif);
    $execute_verif = $prepare_verif->execute([$login]);
    $nb = $prepare_verif->rowCount();
    if($nb == 0){
        $requete = "INSERT INTO coiffeur (login_coiffeur, mot_pass_coiffeur, nom_coiffeur, prenom_coiffeur, email_coiffeur, telephone_coiffeur, datenaiss_coiffeur) VALUES (?,?,?,?,?,?,?)";
...
<div class="container my-3 w-75 mx-auto">
    ...
    <h1 class="text-uppercase my-4 text-center">Ajouter Coiffeur</h1>
        <a href="gest_coiffeur.php" class="btn btn-primary">Liste Coiffeurs</a>
    </div>
    <?php if($success): ?>
        <div class="alert alert-success" role="alert">
            Ajout du coiffeur effectuer avec success
        </div>
    <?php endif; ?>
    <?php if($erreur): ?>
